...
for spk_id in spk_iter:
    ...
    for i, audio_name in enumerate(audio_iter):
        ...
        test_utt_path.append(path)
        test_utt_id.append(utt_id)
...
for spk_id in spk_iter:
    ...
    for i, audio_name in enumerate(audio_iter):
        utt_id = audio_name.split(".")[0]
        path = os.path.join(spk_dir, audio_name)
        illegal_utt_path.append(path)
        illegal_utt_id.append(utt_id)
        illegal_spk_id.append(spk_id)

audio_path_list = (enroll_utt_path + z_norm_utt_path + test_utt_path + illegal_utt_path)
